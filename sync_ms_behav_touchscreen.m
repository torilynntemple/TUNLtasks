...
folder = dir(pwd);
filePrefix = 'msCam';
Prefix_TUNL = '030803 August 13.dat'; 
...
%---------------I DUB THIS HOT MESS.---------------------------------------
%filling the frame map <-- what's a frame map?
%as of right now we hate this chunk of code. 
...
%looks for the first TTL in item_name to show "on" time and record when
%that was. Then it'll look for the second TTL in item_name and that is when
...
for i = startInd : length(eventInd)
    if abs(ind - eventInd(i))>0 %why this??
        ...
        elseif ~isempty(find(contains(eventname(group),'Touch Down Event'),1))
            ...
            elseif ~isempty(find(contains(item_name(group2),'Correct'),1))&& ~isempty(find(contains(item_name(group),'Start Delay'),1))
                ...
                events(eventInd(i),4) = strtok(a);
                events(eventInd(i),6) = 1;
            elseif ~isempty(find(contains(item_name(group2),'Incorrect'),1)) 
                events(eventInd(i),2) = timeMap(eventInd(i));
                events(eventInd(i),3) = 'Nose-Poke Incorrect';
                [a,b]=strtok(arg(group(find(contains(eventname(group),'Touch Down Event')+i-1,1))),['Image' 'Position']);            
                events(eventInd(i),4) = strtok(a);
            elseif ~isempty(find(contains(item_name(group2),'Correct'),1))
                events(eventInd(i),2) = timeMap(eventInd(i));
                events(eventInd(i),3) = 'Nose-Poke Correct';
                [a,b]=strtok(arg(group(find(contains(eventname(group),'Touch Down Event')+i-1,1))),['Image' 'Position']);            
                events(eventInd(i),4) = strtok(a);
        ...
        if ~isempty(find(contains(item_name(group),'Next trial'),1))             
